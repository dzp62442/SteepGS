...
- 结果 protocol 指纹与当前有效参数、seed、split 内容及顺序、适配代码、SteepGS commit/submodule 和关键依赖版本一致；
...
skip 时不重写 completion marker。任何缺件、NaN、不可解码图片、指纹漂移或目录中混入不匹配协议，都判定为未完成，并打印明确拒绝原因。
...
协议文件同时保存人可读配置和规范化后的 SHA-256。至少纳入：

- Center150 JSON 内容、顺序和数据版本；
- 每场 6/18 个 view ID、RGB/K/c2w 及初始化 depth/conf 身份；
- 分辨率、置信度阈值、迭代数、里程碑、seed；
- 所有生效的模型/优化/渲染参数；
- SteepGS commit、dirty 状态、相关适配文件摘要和 submodule revision；
- Python、PyTorch、CUDA、LPIPS backbone/权重版本。

指纹不匹配时拒绝复用结果，并要求使用由协议生成的新目录名或显式指定新的 `--result-root`。
...
- 预处理缓存指纹、临时目录发布、协议冲突、安全清理、完整 skip、不完整重跑和 150 场景汇总均有测试；
